...
export const countryCodeIndexMap = {}
...
countries.map((c, i) => {
    ...
    if (c[3]) {
        countries[i].format = c[3]
    }

    if (c[5]) {
        countries[i].hasAreaCodes = true
        c[5].map((areaCode, j) => {
            const dialCode = c[2] + c[5][j]
            addCountryCode(c[1], dialCode)
        })
    }
    countryCodeIndexMap[countries[i].iso2] = i
...
// WEBPACK FOOTER //
// ./app/constants/country-codes.js