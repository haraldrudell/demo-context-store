  ...
  renderStage = (exec, stage, idx) => {
    ...
    if (stage.status === 'QUEUED') {
      ...
      if (stage.estimatedTime) {
        timeValue = <div>{Utils.formatDuration(stage.estimatedTime / 1000)}</div>
      } else {
        timeValue = <div>-</div>
      }
      ...
      timeTitle = <div className="light">{text}</div>
      timeValue = <div>{Utils.formatDuration((stage.endTs - stage.startTs) / 1000)}</div>
    } else {
        ...
        <div className={'row'}>
          {executions &&
            ...
            executions.map(exec => {
              ...
              if (showingWorkflow) {
                if (showingWorkflow.stateName === 'Approval') {
                  workflowCard = (
                    <div>
                      <ApprovalCardView
                        exec={exec}
              ...
              if (Utils.isRunning(exec.status)) {
                ...
                totalTimeValue = <div>{Utils.formatDuration(exec.estimatedTime / 1000)}</div>
              } else {
                totalTimeTitle = <div className="light">Total Time</div>
                if (exec.endTs === null) {
                  totalTimeValue = <div>{Utils.formatDuration((exec.createdAt - exec.startTs) / 1000)}</div>
                } else {
                  totalTimeValue = <div>{Utils.formatDuration((exec.endTs - exec.startTs) / 1000)}</div>
                }
              }
                  ...
                  <div className={'box-solid wings-card __card'}>
                    <div className="box-header with-border">
                      <div className="wings-card-header">
                        <div>{exec.pipeline.name}</div>
                        <span className="__execStatus">
                          <TimeAgo date={exec.createdAt} minPeriod={30} />
                          <span> - </span>
                          <span>{Utils.formatDate(exec.lastUpdatedAt)}</span>
                          <span> - </span>
                    ...
                    <div className="box-body">
                      <div className="__pipes">
                        ...
                        <div className="wings-card-col __col">
                          ...
                          <div className="__execPipeStatus">{CompUtils.renderStatusText(exec.status)}</div>
                        </div>
                      </div>
...
// WEBPACK FOOTER //
// ../src/pages/pipelines/views/PipelineExecCardView.js