	...
	$(function() {
		...
		*/
		function init(element, opts) {
			...
			$.getJSON(options.baseURL + sliders.twitter.uricb,
				util.format(sliders.twitter.uri, options.username, options.slides),
				tweetResult)

			function tweetResult(data) {
				if (!Array.isArray(data)) alert('twitter response problem')
				else {
					var tweetsHtml = tweetsJsonToHtml(data)

					// insert the list of tweets into the page so that we can figure out how high they are in pixels
					element.empty()
					element.append($(tweetsHtml))
					...
					if (element.height() > element.parent().height()) { // we need to scroll
						...
						element.append($(tweetsHtml).clone())
