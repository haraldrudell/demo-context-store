...
exports['Option NonEnumerable:'] = {
	...
	'Buffer': function () {
		if (Buffer) { // we have a Buffer constructor
			/*
			The bytes of the buffer are undefined, so we have to initialize them
			a parent property references a parent SlowBuffer
			*/
			...
			expect.push(util.format('\n  parent: object:SlowBuffer {\n    0: 5,\n    1: 6,\n    2: 7,\n    3: 8,\n    4: 9,\n    ...,\n    %s: 99', input.parent.length - 1))
			expect.push(util.format('\n    used: %s', input.parent.used))
			expect.push(util.format('\n    length: %s', input.parent.length))
			expect.push(util.format('\n  offset: %s', input.offset))

			var actual = inspect(input, {nonEnum:true, maxProperties: 5})

			expect.forEach(findExpect)

			function findExpect(s, index) {
				assert.ok(~actual.indexOf(s), 'index: ' + index + ' string: ' + s)
			}
		}
	...
	'PrototypeChain No properties': function () {
...
exports['Regressions:'] = {
	'JsUtil Array': function () {
		var actual = inspectSingle([1])
...
n: number, >0, 32 bit: number of spaces
return value: string of n spaces
*/
function spaces(n) {
	var result = []
	var spaces = '\u0020'
