...
var Process_1 = createCommonjsModule(function (module) {
'use strict';
...
Process.instantiate = args => (async () => {

  // flags to ignore if import and not a command-line invocation
  const options = args || {};
  if (options.hasOwnProperty('require') && (!options.require || options.require.main !== options.module)) return;

  // global error handlers
  const pd = Process.processData;
  if (!options.allowUncaught && !pd.uncaughtHandler) process.on('uncaughtException', pd.uncaughtHandler = Process.uncaughtException);
  if (!options.allowReject && !pd.rejectHandler) process.on('unhandledRejection', pd.rejectHandler = Process.unhandledRejection);
...
Process.uncaughtException = e => Process.exit(e, 'uncaughtException');

Process.errorHandler = e => Process.exit(e, 'errorHandler');

Process.unhandledRejection = e => Process.exit(e, 'unhandledRejection');

var Process$1 = Process.instantiate;

module.exports = Process$1;
});

module.exports = Process_1;
